...
void solveSSNQP(double** Q, double *q, int classID, int classNumber, 
        double penalty, int k, double tolorance, int maxIter, int verbose,
...
public:
    ...
    thread_data(double **HH, double **hh, double* labels, 
            int classNumber, double penalty, int k, 
            double tolorance, int maxIter, int verbose,
            double **xx, double *iter, int start, int end, 
            double *sqrtH, double *iSqrtH){
        this->HH = HH;
        this->hh = hh;
        this->labels = labels;
    ...
    void run(){
        ...
        for (int i=start; i < end; i++) {
            double it = 0;
            solveSSNQP (HH, hh[i], (int)labels[i], classNumber, 
             penalty, k, tolorance, maxIter, verbose, xx[i], 
                    &it, &stopMax, sqrtH, iSqrtH, this);
            *(iter) += it;
        }
    }
...
void solveSSNQP(double** Q, double *q, int classID, int classNumber, double penalty, 
        int k, double tolorance, int maxIter, int verbose,
        double *x, double* iter, double *stopMax, double* sqrtH, double* iSqrtH, thread_data* args){
    double *df = args->df;
    ...
    double *dx = args->dx;
    For(i, k)
        if (classID != i/classNumber) q[i] += penalty;
