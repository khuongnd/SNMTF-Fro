function [W, H, HIS] = SNMTFFro(V, opts)
...
%     end
    ...
    [W, iterW] = doSNQP(HH, VH, W, maxIterW, tolW, 0, maxNumberThreads);
    %fprintf('Learing: %.4E\n\n', norm(V-W*H, 'fro')^2);
    ...
    if isfield(opts, 'beta_1'), WW = WW + 2 * opts.beta_1 * eye(k); end
    
    [H, iterH] = doSNQP(WW, VW, H, maxIterH, tolH, 0, maxNumberThreads);

    Q = (W*W').*(H*H');
    D(:) = sum(sum(V.*(W'*H))) / sum(sum(Q));
    %sum(D)
    HIS.time = [HIS.time toc(startTime)];
    HIS.error = [HIS.error norm(V - D(1)*W'*H, 'fro')^2/2.0];
    ...
    if isfield(opts, 'beta_1'), HIS.error(i+1) = HIS.error(i+1) + norm(W, 'fro') * opts.beta_1; end;
    if isfield(opts, 'mu_2'), HIS.error(i+1) = HIS.error(i+1) + sum(sum(H)) * opts.mu_2; end;
    if isfield(opts, 'beta_2'), HIS.error(i+1) = HIS.error(i+1) + norm(H, 'fro') * opts.beta_2; end;
    %if opts.verbose, fprintf('%.10E\n', HIS.error(i+1)); end;
     
    if opts.verbose,
        fprintf('%d,\t%.4f,\t%.4E\n', ...
            i, HIS.time(i+1), HIS.error(i+1));
    end
end
W=W';
HIS.D = D;
